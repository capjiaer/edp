...
# need add more variables
    ...
    if {[regexp {^s} $syn_dc(techlib,process)} {
    ...
    if {[regexp {^t} $syn_dc(techlib,process)} {
	    set_pin_name_synonym D next_state
	...
	set_app_var uniquify_naming_style "$syn_dc(desname)_%s_%d"
	...
	if {[info exits $syn_dc(datain,upffile)] && ($syn_dc(datain,upffile) ne "")} {
		if {[file exists $syn_dc(datain,upffile)]} {
...
#		set_app_var timing_pocvm_enable_analysis true 
    ...
    if { $syn_dc(flow,optstrategy) eq "timing"} {
		if {[shell_is_in_topographical_mode]} {
	...
	set_app_var compile_seqmap_enable_output_inversion false
	 ...
	 if {[info exists $syn_dc(flow,ungroupdw)] && !$syn_dc(flow,ungroupdw)} {
	 ...
	 if {[info exits $syn_dc(config,keep_redundant_regs)] && ($syn_dc(config,keep_redundant_regs) == 1)} {
 ...
 # set_app_var enable_register_transformation_log true
     ...
     #file normalize $syn_dc(datain,upf_file) 
     if {[info exists $syn_dc(datain,upf_file)] && [file exists $syn_dc(datain,upf_file)]}
        set_app_var hdlin_enable_upf_compatible_naming true
     }
